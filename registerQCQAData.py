...
shrtOpts = 'hdif:l:'
longOpts = ['help', 'debug', 'int2r', 'file=', 'location=']
...
if not debug:
    mtdcdb.initiateSession(user='organtin', proxy = False)
...
# dump the XML files
mtdcdb.transfer(condition1, dryrun = dryrun, user='organtin', testdb = testdb)
if debug:
    print(mtdcdb.mtdxml(condition1))

mtdcdb.transfer(condition2, dryrun = dryrun, user='organtin', testdb = testdb)
if debug:
...
for barcode, status in status.items():
    mtdcdb.transfer(status, dryrun = dryrun, user='organtin', testdb = testdb)
    if debug:
