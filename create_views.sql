...
CREATE VIEW PARTSVIEW AS
       SELECT A.*, REGEXP_REPLACE(A.DISPLAY_NAME, ' .*', '') AS GTYPE FROM ALLPARTS A INNER JOIN
       (SELECT BARCODE, MIN(ATTRIBUTE_NAME) AS ATTRIBUTE_NAME FROM ALLPARTS GROUP BY BARCODE) B
       ON A.BARCODE = B.BARCODE AND A.ATTRIBUTE_NAME = B.ATTRIBUTE_NAME WHERE NAME = 'PART_REGISTRATION';
