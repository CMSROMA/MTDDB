...
SELECT P.BARCODE, P.SERIAL_NUMBER, P.BATCH_NUMBER, P.COMMENT_DESCRIPTION,
       ...
       P.RECORD_INSERTION_TIME, KC.NAME, KC.IS_RECORD_DELETED, CR.RUN_TYPE,
...
CREATE VIEW PARTSVIEW AS
       SELECT A.*, SUBSTR(LOWER(REGEXP_REPLACE(A.DISPLAY_NAME, '( .*|v.*)', '')), 0, 13)
...
SELECT P.BARCODE, PC.BARCODE AS CHILD, K.DISPLAY_NAME AS TYPE, 
       SUBSTR(LOWER(REGEXP_REPLACE(K.DISPLAY_NAME, '( .*|v.*)', '')), 0, 13) AS GTYPE,
       ...
       WHERE P.IS_RECORD_DELETED = 'F' AND PT.IS_RECORD_DELETED = 'F' AND PC.IS_RECORD_DELETED = 'F';
